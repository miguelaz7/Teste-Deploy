...
@Repository
public interface RepositorioEventoValidacao extends JpaRepository<EventoValidacao, Long> {

    boolean existsByIngestionHashAndIngestedAtAfter(String ingestionHash, OffsetDateTime reference);

    List<EventoValidacao> findByOriginStop_StopId(String stopId);

    long countByIngestedAtAfter(OffsetDateTime reference);

    long countByPerfilClassificado(String perfilClassificado);

    List<EventoValidacao> findByPerfilClassificado(String perfilClassificado);

    List<EventoValidacao> findByPerfilClassificadoIsNullOrPerfilClassificado(String perfilClassificado);

    List<EventoValidacao> findByPerfilClassificadoAndTransactionDateTimeBetween(
        String perfilClassificado, OffsetDateTime inicio, OffsetDateTime fim);

    List<EventoValidacao> findByTicketType_Code(String code);

    @Query("SELECT e.routeId, COUNT(e) FROM EventoValidacao e WHERE e.ingestedAt > :reference GROUP BY e.routeId")
    List<Object[]> countByRouteIdAfter(@Param("reference") OffsetDateTime reference);

    @Query("SELECT e.ticketType.code, COUNT(e) FROM EventoValidacao e WHERE e.ingestedAt > :reference GROUP BY e.ticketType.code")
