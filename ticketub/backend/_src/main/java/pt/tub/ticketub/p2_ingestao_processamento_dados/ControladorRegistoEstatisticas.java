...
class ControladorRegistoEstatisticas {
    ...
    ControladorRegistoEstatisticas(RepositorioEstatisticasLoteIngestao batchStatsRepository,
    ...
    void register(String batchId, int total, int validos, int invalidos,
        ...
        boolean qualityAlert = total > 0 && ((double) invalidos / total) > LIMIAR_QUALIDADE;
        ...
        batchStatsRepository.save(new EstatisticasLoteIngestao(
            batchId,
            OffsetDateTime.now(),
        ...
        audit("INGESTION_CYCLE_STATS", "batchId", batchId,
            String.format("total=%d validos=%d invalidos=%d duplicados=%d tempoMs=%d alerta=%b",
                total, validos, invalidos, duplicados, tempoMs, qualityAlert));

        // Alerta de qualidade de dados (UC02.4)
        if (qualityAlert) {
            audit("DATA_QUALITY_ALERT", "taxaFalha",
