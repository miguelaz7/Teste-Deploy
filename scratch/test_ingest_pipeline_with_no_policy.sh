...
# Clear existing events and delete all policies
echo "=== Clearing DB validation events, data lake, quarantine, and policies ==="
docker exec -i tub-database mysql -u root -ppassword tub_pgu -e "DELETE FROM validation_events; DELETE FROM ngsi_ld_data_lake; DELETE FROM validation_quarantine; DELETE FROM politicas_anonimizacao;"
